...
(function () {
  ...
  function nutrition(hb) {
    ...
    var meal = function (slot, m) { if (!m) return "";
      var txt = '<div class="text-[10px] uppercase tracking-wide text-[#0a6cf5] font-semibold">' + slot + '</div><div class="text-sm font-semibold text-[#16201d]">' + esc(m.name) + ' <span class="text-[#9aa0ab] font-normal">· ' + esc(m.calories) + " kcal · " + esc(m.protein_g) + 'g P</span></div><div class="text-xs text-[#6b7280]">' + esc(arr(m.ingredients).join(", ")) + (m.serving ? " · " + esc(m.serving) : "") + '</div>' + (m.prep ? '<div class="text-xs text-[#9aa0ab] italic">' + esc(m.prep) + "</div>" : "");
      var img = m.image ? '<img src="' + esc(m.image) + '" alt="" loading="lazy" class="hb-mealimg w-14 h-14 rounded-lg object-cover shrink-0 border border-[#eee]">' : "";
      return '<div class="border-l-2 border-[#d6e6ff] pl-3 py-1' + (m.image ? " flex gap-2.5 items-start" : "") + '">' + img + (m.image ? '<div class="min-w-0">' + txt + "</div>" : txt) + "</div>"; };
  ...
  function workout(hb) {
    ...
    var phases = arr(w.phases).map(function (p, i) {
      var sd = p.sample_day || {};
      var ex = arr(sd.exercises).map(function (e) {
        var txt = '<div class="text-sm font-semibold text-[#16201d]">' + esc(e.name) + ' <span class="text-[#9aa0ab] font-normal">· ' + esc(e.sets) + " × " + esc(e.reps) + (e.rest_sec ? " · rest " + esc(e.rest_sec) + "s" : "") + '</span></div>' + (e.instructions ? '<div class="text-xs text-[#6b7280]">' + esc(e.instructions) + "</div>" : "") + '<div class="text-[11px] text-[#9aa0ab]">' + (e.easier ? "<b>Easier:</b> " + esc(e.easier) + " · " : "") + (e.harder ? "<b>Harder:</b> " + esc(e.harder) : "") + "</div>" + (e.safety ? '<div class="text-[11px] text-[#9aa0ab]"><b>Safety:</b> ' + esc(e.safety) + "</div>" : "");
        var img = e.image ? '<img src="' + esc(e.image) + '" alt="" loading="lazy" class="hb-eximg w-14 h-14 rounded-lg object-cover shrink-0 border border-[#eee] bg-white">' : "";
        return '<div class="border-l-2 border-[#d6e6ff] pl-3 py-1' + (e.image ? " flex gap-2.5 items-start" : "") + '">' + img + (e.image ? '<div class="min-w-0">' + txt + "</div>" : txt) + "</div>"; }).join("");
